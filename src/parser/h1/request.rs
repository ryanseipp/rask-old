...
fn parse_target<'b>(buf: &mut RawRequest<'b>) -> ParseResult<&'b str> {
    ...
    if !found {
        found = unsafe { parse_target_vectorized_avx2(buf) };
    }
    ...
    if !found {
        unsafe { parse_target_vectorized_ssse3(buf) };
    };
...
fn get_header_name<'b>(buf: &mut RawRequest<'b>) -> Option<&'b [u8]> {
    ...
    if !found {
        found = unsafe { validate_header_name_avx2(buf) };
    }
    ...
    if !found {
        found = unsafe { validate_header_name_ssse3(buf) };
    }
...
fn get_header_value<'b>(buf: &mut RawRequest<'b>) -> Option<&'b [u8]> {
    ...
    if !found {
        found = unsafe { validate_header_value_avx2(buf) };
    }
    ...
    if !found {
        found = unsafe { validate_header_value_ssse3(buf) };
    }
